...
bool testPassword(int beginRange, int endRange, char* password) {
    ...
    **/
    ...
    bool hasGroupOfTwo = false;
    for (int i=0; i<len-1; i++) {
        char current = password[i];
        char next = password[i+1];

        //adjacent check
        bool adjacent = (current == next);
        if (adjacent) {
            currentGroupSize++;
        }
        
        //do some 'edge detection'. when an edge is detected, check the group size. if there's
        //at least one group of two, then we pass the adjacency check
        bool endOfGroup = (!adjacent || (i==len-2));
        if (endOfGroup) {
